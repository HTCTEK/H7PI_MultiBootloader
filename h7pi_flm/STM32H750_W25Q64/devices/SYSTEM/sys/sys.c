...
u8 Sys_Clock_Set(u32 plln,u32 pllm,u32 pllp,u32 pllq)
{
	...
	tmpreg = PWR->D3CR & (3<<14); //Delay after an RCC peripheral clock enabling
	((void)tmpreg);
...
#else
	RCC->CR |= ((3<<3) | 1); //HSION=1, HSIDIV[1:0]=3, 8分频
	while(((RCC->CR & (1<<2)) == 0) && (retry < 0x7FFF))
	{
		retry++; //等待HSI RDY
	}
	if (retry == 0X7FFF)
	...
	RCC->ICSCR = ((RCC->ICSCR & (0x3F << 12)) | (0x20 << 12)); //HSI时钟微调
#endif
	
	//RCC->CR &= ~(1<<24); //Disable the main PLL
	//while((RCC->CR & (1<<25)) != 0); //PLL1RDY=0?,等待PLL1准备好
	
#if (PLL_SRC)
	RCC->PLLCKSELR |= 2<<0; //PLLSRC[1:0]=2,选择HSE作为PLL的输入时钟源
#else
	RCC->PLLCKSELR |= 0<<0; //PLLSRC[1:0]=2,选择HSI作为PLL的输入时钟源
#endif
	...
	//RCC->PLL1FRACR = (frac & 0x1FFF) << 3); //设置PLL1的小数分频器
